...
<%
    ...
    Map<String, String> itemPurchaseMap = new LinkedHashMap<>();
    ...
    for (String[] item : supplierItems) {
        ...
        String purchase = item[5].trim();
        itemExpiryMap.put(itemName, expiry);
        itemPurchaseMap.put(itemName, purchase);
    }
%>
...
<head>
    ...
    <script>
        ...
        <% for (Map.Entry<String, String> entry : itemPurchaseMap.entrySet()) { %>
        itemPurchaseMap["<%= entry.getKey() %>"] = "<%= entry.getValue() %>";
        <% } %>
        ...
        function updateCurrentExpiry() {
            ...
            var newExpiryField = document.getElementById("newExpiry");
            var selectedItem = select.value;
            currentExpiryField.value = itemExpiryMap[selectedItem] || "";
            var purchaseDate = itemPurchaseMap[selectedItem];
            if (purchaseDate) {
                newExpiryField.min = purchaseDate;
            } else {
                newExpiryField.removeAttribute("min");
            }
        }
    </script>
</head>
